...
for i,tool in enumerate(cuttool):
    if redo:
        i +=1
        with open(f"index_freq/{tool}.json","r") as f:
            data = json.load(f)
        ...
        dff.to_csv(f"index_freq/freq_{tool}.csv",index=0)
        z_data = dff
        z = z_data.values
...
fig.update_layout(title=f'Time-frequency x 加工次数,y 频率K,z 幅值', autosize=False,
